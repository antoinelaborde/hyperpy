...
from hyperpy.loading.utils import read_raw, read_specim, get_reflectance, add_prefix_filename, expand_average, \
    read_hyspex, get_wavelength
...
class TestReadSpecim:
    ...
    def test_read_specim(self, mocked_read_raw, mocked_expand_average, mocked_get_reflectance, mocked_get_wavelengths):
        ...
        mocked_expand_average.assert_has_calls([
            mock.call(mocked_white_ref, 10),
            mock.call(mocked_black_ref, 10)
        ])
...
class TestExpandAverage:
    def test_expand_average(self):
        ...
        average_expanded = np.array([[[4., 16.],
                                      [4., 16.]],
                                     [[5., 17.],
                                      [5., 17.]],
                                     [[6., 18.],
    ...
    class TestReadHyspex:
        ...
        @mock.patch('hyperpy.loading.utils.get_reflectance')
        @mock.patch('hyperpy.loading.utils.read_raw')
        def test_read_hyspex(self, mock_read_raw, mock_get_reflectance, mock_get_wavelength, mock_expand_average):
            mocked_raw = MagicMock(shape=[15, 10])
            ...
            mock_get_wavelength.assert_has_calls([
                mock.call('filename')
            ])
