...
transformer_class_list = [
    class_module[0]
    for class_module in inspect.getmembers(transformers, inspect.isclass)
]
...
# Add widgets
select_processing = Select(
    title="Preprocessing:", value=transformers_name[0], options=transformers_name
)
...
slider_window = Slider(
    start=3, end=66, value=7, step=2, title="Window Size Savitsky-Golay"
)
slider_polynomial = Slider(
    start=1, end=4, value=1, step=1, title="Polynomial Order Savitsky-Golay"
)
slider_derivation = Slider(
    start=0, end=3, value=0, step=1, title="Derivation Order Savitsky-Golay"
)
    ...
    for transformer_name in multi_select.options:
        ...
        if transformer_name == "Savistky Golay filter":
            transformer_instance = transformer(
                ...
                polynomial_order=slider_polynomial.value,
                derivation_order=slider_derivation.value,
            )
